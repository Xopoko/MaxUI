...
public struct MToggle: Componentable {
    ...
    /// Initialise a new `MToggle` instance.
    ///
    /// - Parameters:
...
extension MToggleView: ReusableView {
    public func configure(with data: MToggle) {
        CancellableStorage(in: &cancellables) { [weak self] in
            data._isOn.weakAssign(to: \.isOn, on: self?.switcher)
            data._isEnabled.weakAssign(to: \.isEnabled, on: self?.switcher)
            data._onSwitch.sink { self?.switchHandler = $0 }
