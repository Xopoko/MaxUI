...
///     .configure(in: self)
///
public struct MVStack: Componentable, DeclarativeCommon {
    ...
    /// Initialise the `MVStack` with a `MView` builder (`MViewBuilder`).
    ///
    ...
    /// Initialise the `MVStack` with a `MView` array.
    ///
